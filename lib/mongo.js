 ...
 *
 * Options:
 *  - ensureIndexes: ensure that all model indices exist before
 *    returning the connection.
 *
 * Return:
 *   A promise that resolves to the opened connection.
 */
var openConnection = exports.openConnection = function(conn, mongodbURL, options) {
    var u = url.parse(mongodbURL, true);
    if (!u.protocol) {
        // Plain database name, so add it to the default connection
        u = url.parse(config.mongodbURL, true);
        u.pathname = mongodbURL;
    }

    if (!u.protocol) {
        throw new Error('cannot parse mongodbURL: ' + config.mongodbURL);
    }

    var ensureIndexes = function(openConn) {
        if (!(options && options.ensureIndexes)) {
            return openConn;
        }

        return Promise.resolve(openConn.modelNames())
            .map(function(name) {
                debug('ensuring indexes on %s.%s', u.pathname, name);
                var model = openConn.model(name);
    ...
    // Set ?authSource= if not already done
    if (!u.query.authSource) {
        u.query.authSource = config.mongodbAuthSource;
        u.search = null; // will otherwise override u.query
    }
